<script lang="ts">
  ...
  async function loadQuote() {
    ...
    try {
      ...
      formData = {
        ...
        effectiveDate: quote.dates?.effective ? formatDateForInput(quote.dates.effective.toDate()) : '',
        expirationDate: quote.dates?.expiration ? formatDateForInput(quote.dates.expiration.toDate()) : '',
        coverageLimits: {
          perOccurrence: quote.coverage?.limits?.perOccurrence?.toString() || '',
          aggregate: quote.coverage?.limits?.aggregate?.toString() || ''
        },
        deductible: quote.coverage?.deductible?.toString() || '',
        notes: quote.notes || ''
      };
    } catch (err) {
  ...
  async function handleSubmit() {
    ...
    // Validation
    if (!formData.coverageType || !formData.effectiveDate || !formData.expirationDate) {
  ...
  <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="mb-8">
      <div class="flex items-center space-x-4">
        ...
        <div>
          <h1 class="text-3xl font-bold text-neutral-900">Edit Quote</h1>
          <p class="text-neutral-600">Modify quote information</p>
        </div>
      </div>
    </div>
        ...
        <div class="px-6 py-6">
          <form on:submit|preventDefault={handleSubmit}>
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <!-- Coverage Type -->
              <div>
                <label for="coverageType" class="block text-sm font-medium text-neutral-700">
                  Coverage Type *
                </label>
                <select
                  id="coverageType"
                ...
                >
                  ...
                  <option value="employment_practices">Employment Practices</option>
